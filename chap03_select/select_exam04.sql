...
select gno as 선물번호, gname as 선물이름 from gift where gno <= 2;
...
select ename,sal,empno from emp where deptno in (30);

-- [문3] student 테이블의 전체 학생들을 대상으로 다음 예)와 같이 출력하기 
-- 예) 홍길동의 키는 175cm, 몸무게는 65kg입니다.
-- 사용 컬럼명 : 키(height), 몸무게(weight)

select name||'의 키는 '||height||'cm, 몸무게는 '||weight||'kg 입니다.' as total from student;
...
select name 이름, height 키 from student order by 이름; 
