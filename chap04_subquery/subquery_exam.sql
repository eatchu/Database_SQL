...
-- 조건)   IN()함수 이용 : 다중 행 처리  
select name,position,pay,bonus from professor where name 
in (select name from professor where bonus is not null);

-- [문4] 301 학과(DEPTNO) 교수들 보다 더 많은 급여를 받는 교수들의 이름, 직위, 급여, 학과 출력
-- 조건) ALL()함수 이용 : 다중 행 처리 
select name,position,pay from professor where  pay > all
